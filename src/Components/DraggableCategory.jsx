...
export default function DraggableCategory({ category, index, onUpdate, onDelete, onCopy, moveCategory }) {
  ...
  const handleAddChild = () => {
    if (newChildName.trim()) {
      const newChild = { id: Date.now().toString(), name: newChildName, children: [], isCopy: false, copyCount: 0, originalId: null }
    ...
    <div ref={(node) => drag(drop(node))} className={borderClass}>
      ...
      <div className="flex items-center mb-3">
        {isEditing ? (
          <input
            ...
            className="flex-grow border-2 border-yellow-300 rounded-lg px-3 py-2 mr-2 focus:outline-none focus:border-yellow-500 transition-all duration-300"
          />
        ) : (
          <span className="flex-grow text-lg font-semibold text-gray-800">{category.name}</span>
        )}
      ...
      <div className="flex mt-3">
        <input
          ...
          className="flex-grow border-2 border-blue-300 rounded-l-lg px-4 py-2 focus:outline-none focus:border-blue-500 transition-all duration-300"
