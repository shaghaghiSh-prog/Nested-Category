...
export default function Home() {
  ...
  const addCategory = () => {
    if (newCategoryName.trim()) {
      setCategories([...categories, { id: Date.now().toString(), name: newCategoryName, children: [], isCopy: false, copyCount: 0, originalId: null }])
      setNewCategoryName('')
    }
  }

  const updateCategory = (updatedCategory) => {
    setCategories(prevCategories => updateCategoryRecursive(prevCategories, updatedCategory))
  }
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="min-h-screen bg-gradient-to-r from-blue-100 to-purple-100 p-8">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-8">
          <h1 className="text-4xl font-bold mb-8 text-center text-gray-800 border-b-2 pb-4">Nested Categories</h1>
          <div className="flex mb-6">
            <input
              type="text"
              value={newCategoryName}
              onChange={(e) => setNewCategoryName(e.target.value)}
              className="flex-grow border-2 border-blue-300 rounded-l-lg px-4 py-3 focus:outline-none focus:border-blue-500 transition-all duration-300"
              placeholder="New category name"
            />
            <button
              onClick={addCategory}
              className="bg-blue-500 text-white px-8 py-3 rounded-r-lg hover:bg-blue-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
            >
              Add
            </button>
          </div>
